...
Everything seems good except for the fact that "dlib" is already in use by someone else on npm. Gah!
...
Array.prototype.remove = function (start, count = 1) {
  if (start < 0) return;
...
Number.prototype.toTimeString = function (format = 'dd:hh:mm:ss', trimLeadingValue = true, omitLeadingZeros = true) {
  // formats:
  // d - single digit day
  // dd - zero padded day
  // h - single digit hour
  ...
  let m  = Math.floor((this / (1000 * 60)) % 60);
  let h  = Math.floor((this / (1000 * 60 * 60)) % 24);
  let d  = Math.floor(this / (1000 * 60 * 60 * 24));
  ...
  let hasMS = time.includes("ms") || time.includes("ms2") || time.includes("ms3");
  let hasS  = time.includes("s") || time.includes("ss");
  let hasM  = time.includes("m") || time.includes("mm");
  let hasH  = time.includes("h") || time.includes("hh");
  let hasD  = time.includes("d") || time.includes("dd");
...
export const isSafe = function (value) {
  return !(_.isNaN(value) || _.isUndefined(value) || _.isNull(value))
};
...
export const percent2 = function (numerator, denominator, maxValue = 1, minValue = 0, ascending = true) {
  return ((maxValue - minValue) * Math.max(0, Math.min(1, Math.abs(Number(!ascending) - (numerator / denominator))))) + minValue;
};

export const pickHex = function (color1, color2, weight) {
...
export const resizeImage = function (imgSourse, targetWidth, targetHeight, keepAspectRatio = true) {
...
export const toString26 = function (num) {
